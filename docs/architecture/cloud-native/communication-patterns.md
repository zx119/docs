...
The book [.NET Microservices: Architecture for Containerized .NET Applications](../../architecture/microservices/index.md), available for free from Microsoft, provides an in-depth coverage of communication patterns for microservice applications. In this chapter, we provide a high-level overview of these patterns along with implementation options available in the Azure cloud.
